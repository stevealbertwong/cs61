...
// Array of struct -> data structure
...
typedef struct _ce {
    ...
    off_t tag; // remaining bits are tag -> supply the rest of the address bits to let us distinguish between different memory locations that map to the same cache block
} cache_entry;

typedef struct _fd {
    cache_entry entries[MAX_CACHE_SIZE]; 
    off_t offset;           // Next file position to read -> // where in the block, determined by block size
