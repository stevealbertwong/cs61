...
void bank_transfer(int acct1, int acct2, int amount) {
    ...
    // Enforce an ordering when acquiring locks
    int small = (acct1 < acct2) ? acct1 : acct2;
    int large = (acct1 > acct2) ? acct1 : acct2;
    pthread_mutex_lock(&locks[small]);
    pthread_mutex_lock(&locks[large]);

    accounts[acct1] -= amount;
    accounts[acct2] += amount;

    pthread_mutex_unlock(&locks[large]);
    pthread_mutex_unlock(&locks[small]);
}
